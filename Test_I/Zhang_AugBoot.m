function [output1,output2,output3,output4] = Zhang_AugBoot(TK,TS,B,n,k,m,p_value)
...
K_Test=zeros(m,1);S_Test=zeros(m,1);
for loop=1:m
% Bootstrap procedure
A=Generating(n,k,B);
...
[~,class]=SP_kmeans(A_new,k+1);%Spectral K-Means 
position=class;
[~,K_rho_new] = MLE(A_new,B_new,n+n_new,k+1,'unknown',position); 
%% AUG Test Statistics 
S_Ln=max(max(abs(S_rho_new)));
K_Ln=max(max(abs(K_rho_new)));
K_Test(loop,:)=K_Ln^2-2*log(2*(n+n_new)*(k+1))+log(log(2*(n+n_new)*(k+1)));
...
fun1= {@(param)(m*log(param(2))-sum(-(K_Test-param(1))./param(2))+sum(exp(-(K_Test-param(1))./param(2))))};
...
%%
TK_Boot=-2*log(2*sqrt(pi))+2*(TK-hat_muK)/hat_betaK;  % For test K
TS_Boot=-2*log(2*sqrt(pi))+2*(TS-hat_muS)/hat_betaS;  % For test g
